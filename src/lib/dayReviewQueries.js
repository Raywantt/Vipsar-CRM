...
const LEAD_NAME_EMBED = 'leads(id, current_stage, bdm_employee_id, parties!party_id(name), sites(nickname, locality, house_no))'
...
export function fetchDayStageChanges({ startISO, endISO }) {
  return fetchAllRows(() =>
    supabase
      .from('stage_history')
      .select(
        'id, lead_id, stage, changed_by, changed_at, leads(id, owner_employee_id, bdm_employee_id, order_value, quote_value, parties!party_id(name), sites(nickname, locality, house_no))',
        { count: 'exact' }
      )
      .gte('changed_at', startISO)
      .lte('changed_at', endISO)
...
export function fetchDayQuotesSent(dateISO) {
  return fetchAllRows(() =>
    supabase
      .from('leads')
      .select('id, owner_employee_id, bdm_employee_id, quote_value, quote_sent_at, parties!party_id(name), sites(nickname, locality, house_no)', {
        count: 'exact',
      })
