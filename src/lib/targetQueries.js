...
import { withoutPoolLeadRows } from './poolLeads'
...
// BDM pool leads are left out unless `includePool` — see src/lib/poolLeads.js.
export function fetchWonStageHistory(includePool = false) {
  return cachedQuery(`stage_history:won:${includePool ? 'pool' : 'no-pool'}`, () =>
    fetchAllRows(
      () =>
        supabase
          .from('stage_history')
          .select('lead_id, changed_at, leads(owner_employee_id, bdm_employee_id, order_value)', { count: 'exact' })
          .eq('stage', 'won')
