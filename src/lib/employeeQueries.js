...
export function fetchActiveSalesExecs() {
  // Cached: this is called on nearly every screen (Dashboard, all four Today
  // variants, My Team, the exec profile) and indirectly by fetchMyTeamExecs /
  // fetchMyManagedExecs below, so it was one of the most-repeated requests in
  // the app. The roster also changes far less often than anything else here.
  return cachedQuery('employees:active-sales-execs', () =>
    fetchAllRows(() =>
      supabase
        .from('employees')
