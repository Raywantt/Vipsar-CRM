...
export function fetchAllEmployees() {
  return supabase.from('employees').select('id, name, role, is_active').order('name')
}
...
// shown to the owner.
export function insertEmployee({ name, role, authUserId }) {
  return supabase
    .from('employees')
    .insert({ name, role, auth_user_id: authUserId || null })
    .select('id, name, role, is_active')
    .single()
}

export function updateEmployeeRole(id, role) {
  return supabase.from('employees').update({ role }).eq('id', id).select('id, name, role, is_active').single()
}
