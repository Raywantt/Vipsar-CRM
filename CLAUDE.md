...
src/
  ...
  lib/          integrations & utilities (supabaseClient.js, sanitizeForIlike.js,
                ...
                section's Dark mode bullet —
                fetchAllRows.js — pages any query past PostgREST's 1,000-row
                cap; every list-returning query in this app goes through it,
                see the Conventions bullet on the row-cap bug —
                lookupQueries.js — fetchAreas/fetchProducts, the two small
                reference-table lookups that used to be duplicated inline in
                LeadDetail.jsx and SiteSearchOrCreate.jsx — `homeTiles.js` is
                deleted, see the Mobile redesign section)
...
* **Dark mode** (`src/lib/theme.js`, section 22 of `vipsar-theme.css`) —
  ...
  **Appearance** card (see the Profile section below) — `system` removes
  ...
  keep it in sync with `theme.js` if that key ever changes.
  **The choice also follows the employee's account now** (added
  2026-08-20, at the owner's request — it used to be pure `localStorage`,
  so it "kept defaulting to System" on any device/browser other than the
  one it was set on) — a new `employee_preferences` table
  (`Schema/migration_employee_theme_preference.sql`, **⚠️ outstanding, not
  yet run**, see Conventions) holds one row per employee. Deliberately a
  new table, not a column on `employees`: that table's own `UPDATE` policy
  is owner-only with no self-update exception (see this file's RLS
  paragraph in Conventions — "a sales exec must never set their own role to
  'owner'"), so a column there would mean only the owner could ever save
  their own appearance choice. `employee_preferences` gets its own
  `push_subscriptions`-style policy instead — own row only, no owner-role
  exception on write, since an owner has no legitimate reason to set
  someone else's theme. `theme.js`'s `fetchAccountTheme`/`saveAccountTheme`
  layer on top of the existing `localStorage` functions rather than
  replacing them: `AuthContext.jsx` fires a best-effort
  `fetchAccountTheme` right after login and applies it if the account has
  one saved (syncing `localStorage` too, so this device's own pre-paint
  script has it next time); `Profile.jsx`'s `handleThemeChange` now also
  calls `saveAccountTheme` alongside the existing local `setTheme` call,
  with a small inline warning ("Saved on this device, but couldn't sync to
  your account.") if that write fails — the local choice still applies
  either way, so a network hiccup (or the migration not having been run
  yet) never blocks the control itself. Verified live against the real dev
  database pre-migration: picking Dark still applies instantly and
  persists on reload, the warning renders exactly as designed, and no
  console error or crash results from the missing table (`fetchAccountTheme`
  swallows a `PGRST205` "table not found" the same way it would swallow
  any other failure). The 11
...
* **New** (blue, light→dark): `calling` → `presentation` →
  `joinery_follow_up`.
* **Warm** (amber, light→dark): `rfq` → `quote_submission`.
...
`LEAD_STAGE_OPTIONS` (the 9 values, in the order above) and
...
entirely (see the migration below). **`measurements` and
`design_discussion` were retired the same way, 2026-09-08** (the owner's
ruling) — `measurements` leads were folded into `joinery_follow_up`,
`design_discussion` leads into `rfq`, via
`Schema/migration_retire_measurements_design_discussion.sql`, which also
redeploys `enforce_owner_only_stage_change()` (the forward-only-for-exec
trigger, see `stageProgress.js` below) with the two removed from its own
funnel-sequence array. `rfq`'s label changed to **"RFQ Raised"** in the
same pass (the value itself is unchanged, so this needed no data
migration — just `LEAD_STAGE_LABELS.rfq`). `FUNNEL_SEQUENCE` is 6 stages
now, not 8. Every `lead.current_stage ?? 'new'`
...
Every Client Meeting belongs to one of two umbrellas, decided by the CRM at
the moment it is logged (2026-09-07, the owner's ruling):

* **Old Meeting** (`client_meeting_old`) — the lead was at **RFQ or later**:
  `rfq`, `quote_submission`, `negotiation`, and also `on_hold`, `won` and
  `lost`. Those last three have no funnel rank of their own (`stageRank`
  returns null for all of them, see `stageProgress.js`) so they're listed
  explicitly rather than ranked — giving them a rank would misrepresent the
  funnel everywhere else that helper is used. On hold was named by the owner
  directly; won/lost follow from "RFQ and above".
* **New Meeting** (`client_meeting_new`) — anything earlier: `calling`,
  `presentation`, `joinery_follow_up`, `measurements`, `design_discussion`.
  An **unset or unrecognised legacy stage falls back to New**, not Old — the
  same "don't invent a confident value" rule `dealValueOrNull` and the
  `closure_probability` display already follow.

The rule is `meetingTypeForStage(stage)`, derived from `FUNNEL_SEQUENCE`'s own
order so it can't drift from the taxonomy, and pinned by
`meetingBucket.test.js` (including that *every* `LEAD_STAGE_OPTIONS` value
lands in exactly one bucket, so a stage added later can't fall through).

**The bucket is FROZEN at logging time, not derived live — this is the whole
point.** An activity records what a rep did on a day. If the bucket were
computed from the lead's *current* stage, a stage move next week would
silently rewrite last month's activity report: a meeting genuinely held while
the lead was at `calling` would retroactively become an Old Meeting. So the
bucket **is** the stored `activity_type`, which also means every
meeting-showing surface splits in two for free — Dashboard's Activity card,
`buildActivitiesAttainPanel`'s contribution breakdown, the Day Review's
day-sheet tags, both activity timelines and the heatmap all derive from
`ACTIVITY_TYPES` and needed **no per-site special-casing at all**. Expect
`migration_client_meeting_buckets.sql`'s STEP 4e to show Old meetings sitting
on below-RFQ leads and vice versa: that is correct, not drift — don't "fix" it.
...
to show). `ACTIVITY_METRIC_OPTIONS` goes 5 → 6, so the heatmap goes to 8
columns; that scales itself, since `DashboardHeatmap` feeds `COLS.length` into
`--vip-heatmap-cols` rather than a number typed into the stylesheet. **Any
`targets` row still keyed on the retired `client_meeting` metric no longer
computes an actual** — it's inert (nothing iterates it) rather than harmful,
but it needs re-entering against whichever umbrella it meant; the migration's
STEP 4d lists them.

**`client_meeting` survives in exactly two places, both deliberate**: as
`PICKABLE_MEETING` (the button/chip value, never stored on an activity, still
legal on `follow_ups` where a reminder has no bucket yet), and as an
`ACTIVITY_LABELS` key so any stray legacy row still renders a label instead of
a raw slug.
...
* **A Client Meeting is stored as one of TWO umbrellas, never as itself**
  (2026-09-07, the owner's ruling). `client_meeting` is gone from
  `ACTIVITY_TYPES` and from `activities.activity_type`'s CHECK; every meeting
  lands as **`client_meeting_old`** ("Old Meeting") or **`client_meeting_new`**
  ("New Meeting"). See the Meeting buckets section below for the rule, why the
  value is frozen rather than derived, and what it fans out to. **The rep still
  taps ONE "Client Meeting" button** — Log Activity's tap-select renders
  `LOGGABLE_ACTIVITY_TYPES` (8 entries, one Client Meeting) while every
  *display* surface renders `ACTIVITY_TYPES` (9 entries, the two buckets).
  Don't collapse those two lists back together: the split exists so the CRM
  classifies the day rather than asking a rep on a phone to do it.
  `resolvedMeetingType` is computed once beside `canSubmit`, not again inside
  `handleSubmit` — the bucket the form PROMISES in its hint and the bucket it
  WRITES have to be the same one, or the confirmation lies. The lead-anchored
  `createFollowUp` call still passes the *pickable* `client_meeting`, which is
  why `follow_ups.activity_type`'s CHECK needed no change at all.
...
* **Needs Attention** (`NeedsAttentionCard.jsx` + `src/lib/attention.js`,
  ...
  **The "Stale Leads" tile on Dashboard's "Right now" strip
  (`RightNowStrip.jsx`, part of the 2026-09-07 time-independent-metrics
  build — see `dashboard-time-independent-metrics-prompt.md`/
  `TIME-INDEPENDENT-METRICS-LOG.md`, repo root) is a THIRD instance of this
  same mistake, found and fixed 2026-09-08.** It reused this bucket's own
  `staleBucket` directly (the ATTENTION_DAYS/14 'stale' entry), so it always
  showed the identical number as Needs Attention's "No activity in 14+ days"
  row, under a plain "Stale" label — reported live when both tiles read 72.
  `EmployeeProfile.jsx`'s "Stale leads" head stat and its "Leads assigned"
  card sub-label had the exact same bug in the other direction: both already
  said "7+ days" in their own copy while reading the 14-day bucket's count.
  Fixed by giving STALE_DAYS its own dedicated bucket computation,
  `computeStale7Bucket`/`computeStale7BucketFromRpc` in `src/lib/attention.js`
  — same shape as the ATTENTION_DAYS 'stale' entry (same on_hold exclusion,
  same GREATEST(last activity, last stage change) touch reference), gated at
  the earlier threshold, with its own `stale_7d` key so it's never confused
  with the queue bucket. `Dashboard.jsx` now computes `stale7Bucket`
  alongside `attentionBuckets` (same fast-RPC-vs-client-fallback split, see
  below) and both `RightNowStrip`'s tile count and its own drill-down read
  it; `EmployeeProfile.jsx`'s local `staleBucket` was switched to
  `computeStale7Bucket` outright, since both places that already read it had
  copy explicitly claiming "7+ days". **The Needs Attention queue's own
  "stale" bucket (ATTENTION_DAYS/14) is completely unchanged** — this only
  gave the earlier STALE_DAYS(7) reading a real, separate number of its own
  instead of silently reusing the 14-day one everywhere it was labelled 7.
  **✅ `Schema/migration_stale_7day_tile.sql` was RUN LIVE 2026-09-08** — it
  adds an `is_stale_7d` flag (and widens the WHERE clause so 7-13-day leads
  are actually returned) to `leads_needing_attention()`, so the fast RPC path
  can answer this without a second full-company query. Confirmed via the
  migration's own verification query, run by the owner in the SQL Editor:
  `is_stale_7d` is present in the function's output, and on a real row (lead
  #159, `Stage Rule Test Site`) `is_stale: true` and `is_stale_7d: true`
  together — the superset relationship holds (anything stale enough for the
  ATTENTION_DAYS/14 queue is also stale enough for this tile). Pinned by 8
  unit tests in `attention.test.js` and a clean build. **Still only checked
  via the SQL Editor, not from a real logged-in session** — that query runs
  as `postgres` with BYPASSRLS, so it proves the function's shape and this
  one row's flags but nothing about RLS-scoped behaviour per role; the
  client-side fallback path (`sales_manager`, or anyone whose RPC call fails)
  and `EmployeeProfile.jsx` (always client-side) needed no migration and are
  correct regardless. Worth a look at the Dashboard "Right now" strip and a
  Sales Exec Profile in a real session before calling this fully confirmed.
  **An `on_hold` lead is excluded from the "stale" bucket outright, however
  long the pause has run** (owner's ruling, 2026-09-07) — On Hold
  deliberately takes a lead off the clock, so no length of pause reads as
  neglect. The moment it resumes (any stage change away from `on_hold`),
  this bucket's clock restarts from that day rather than picking up
  wherever it stood before the pause — a stage change counts as a touch,
  via `stage_history`, the same way LeadDetail's own health pill has always
  folded stage changes into its "last touched" calc. See
  `src/lib/attention.js`'s `buildLastStageChangeByLead`/`staleSinceTouch`
  and the `✅ migration_needs_attention_hold_exclusion.sql` Conventions
  entry (run live 2026-09-08 — the fast RPC path now matches the
  client-side fix). Won/lost leads were already excluded
  from every bucket here via `isOpen`; the part that wasn't fixed until the
  same pass was `LeadDetail.jsx`'s health pill and "Last touch" stat, which
  showed a staleness-colored day count for a won/lost/on-hold lead
  regardless — see that page's own `showTouchHealth` guard.
...
* **Set a target** (`SetTargetForm.jsx`, inside the same card, collapsed
  ...
  default) — employee/period_type/period_value/metric_name/target_value,
  the only way to populate the `targets` table. Gated purely by
  `showByEmployee` (`Dashboard.jsx`'s `seesOthersData`) inside
  `TargetsVsActualsCard.jsx` — there's no separate `isOwner` check on this
  button, which is why it quietly stopped being owner-only the moment
  `seesOthersData` grew to include a coordinator and a manager-in-Team-scope
  during the Sales Manager build (2026-09-03): both roles could already open
  the form from that date on, with the employee dropdown already correctly
  narrowed to their own team by the same roster-scoping effect every other
  team-scoped card on this page reads. **What was actually missing sat one
  layer down**: `targets`' RLS only ever granted INSERT/UPDATE to the owner
  or to `employee_id = current_employee_id()`, so tapping "Set" as a
  coordinator or manager hit a real RLS-denied error on a button the UI
  never hid — closed by `Schema/migration_targets_team_write.sql`
  (2026-09-07, see Conventions), which adds team-scoped INSERT/UPDATE
  policies alongside the SELECT ones `migration_sales_coordinator.sql`/
  `migration_sales_manager.sql` already had. This still doesn't let a
  coordinator or manager set a target for *themselves* — the button simply
  never renders for that case (`showByEmployee` is false whenever a manager
  is on the "My" side of their switch, and a coordinator carries no personal
  target row to begin with), unrelated to and unchanged by this migration.
  `period_type` is
  ...
  bullet on `migration_targets_unique.sql`), and appending its result
  regardless would leave the old, superseded row sitting in local state
...
* **A target set for NEXT week really did show up under the CURRENT week —
  ...
  **Verified live** (owner session, real dev database, desktop 1047px and the
  ...
  test rows were deleted afterwards. **The role matrix was then closed on
  2026-09-11** — see the role-pass bullet below.
* **"+ Set a target" now shows what's ALREADY set for the period the stepper
  ...
  **Verified live** (owner session, real dev data, desktop and the mobile
  ...
  Re-verified for the other two roles the same day — see the role-pass bullet
  below.
* **The form opens on the period the dashboard is SHOWING, not a hardcoded
  ...
  **Seeded, not controlled** — they're `useState` initialisers, so changing
  ...
  `quarter` / "1 Jul – 30 Sep 2026 (Q3)". Re-verified for the other two roles
  the same day — see the next bullet.
* **All three target-setting roles were walked, both breakpoints
  (2026-09-11)** — closing the matrix for the three changes above rather than
  leaving them on "same component, so it should hold". Real sessions on the
  three role ports; **key off `.vip-sidebar-foot-role`, not the port name**,
  as the Local environment notes warn.
  - **`sales_coordinator`** (`sc`, id 25; 904px, the mobile branch): card and
    "+ Set a target" render; the form seeds `week` / "7 – 13 Sep 2026" on the
    Week preset and `month` / "September 2026" on Month; the employee
    dropdown and the "Already set for" readout are both scoped to their one
    report, showing "Nothing set for this period yet." where the OWNER sees
    "17 targets across 4 people" for the identical week — which is the check
    that proves the roster scoping, since an unscoped readout would have
    leaked the other four people.
  - **`sales_manager`** (`sm`; 1088px, the real desktop heatmap): "+ Set a
    target" correctly does **not** render on the "My numbers" side of the
    My/Team switch (a manager carries no personal target) and does on "My
    team"; seeds `week`, `month` and `quarter` correctly.
  - **The merge fix holds for both.** Each saved a target for NEXT week and
    watched this week's view stay untouched, with the "Saved for …, the table
    above is showing …" line firing. A nice cross-role confirmation fell out
    of it: the manager's readout, stepped to next week, showed the Call
    target the COORDINATOR had written minutes earlier — real shared data,
    correctly scoped, not a local echo.
  - Both saves also **succeeded**, which is what confirmed
    `migration_targets_team_write.sql` is live (see Conventions — this file
    had it marked OUTSTANDING, and an owner session structurally could not
    have told the difference).
  - Cleanup: both rows deleted **from an owner session** — `targets` DELETE
    is owner-only with no team exception, so neither role can remove its own
    test data. `targets` was confirmed back to its pre-test count (28 week
    rows, highest id 177) rather than assumed.
...
* **A sales exec's `parties`/`sites` read is now scoped to their own leads**
  ...
  **Exception, added 2026-09-10 (⚠️ outstanding — see the Conventions
  bullet on `migration_architects_universal_visibility.sql`):** a party
  with `party_type IN ('architect','firm')` is visible to **every** active
  employee regardless of team/lead ownership — the owner's ruling, since an
  architect/firm is shared infrastructure that routinely appears across
  different reps' and teams' leads, and team-scoped visibility meant a rep
  who couldn't see one just created a duplicate. Edit rights did **not**
  widen to match: the existing creator-or-owner rule
  (`own_data_or_owner_role_update`) is unchanged, and a coordinator's
  separate team-fallback edit policy (`coordinator_team_update`, added by
  `migration_coordinator_entry.sql`) was narrowed to exclude architect/firm
  rows — so for one you didn't create, only the owner can edit it, same as
  every other role. `sales_manager` was explicitly ruled out for any special
  edit right here too (they already had none on `parties`).
...
| reassign | within their team | **any active exec, plus onto themselves** (widened 2026-09-07, see Conventions) |
...
**Known gaps, not oversights:** the "Reassign owner" dropdown lists every
active rep rather than only the supervisor's own team — for a **coordinator**
this is still a real, unfixed gap: the database refuses an out-of-team
target, so the dropdown showing everyone is misleading for that role.
**For a manager this stopped being a gap on 2026-09-07** — reported directly
("in the interface it shows all the execs but when they click on an exec
outside their team, it simply declines the reassignment. make it such that
they can do that") and fixed by `Schema/migration_manager_reassign_any_employee.sql`
(see Conventions), which widens `manager_team_update`'s `WITH CHECK` so a
manager can hand a lead they can already reach (their own, or their team's)
to **any active exec in the company**, not just their own team — the
dropdown already listing everyone was always the intended shape for this
role, the database just hadn't matched it. **No visibility changed by this**
— a manager still only ever reaches a lead through the exact same
`manager_team_select`/`own_data_or_owner_role_select` scoping as before;
only which employee it can be handed *to* widened. A version of this fix
that also made `leads` SELECT unconditional for managers (so they could find
a non-team lead to begin with, matching the owner outright) was drafted and
explicitly rejected — it would have made All Leads and Search go
company-wide for a manager too, which was never asked for. The two real
managers still own no leads (their data arrives with the Excel import), so
the populated "My day" half was verified only through the dummy `sm`
account.
...
- **✅ `Schema/migration_client_meeting_buckets.sql` was RUN LIVE 2026-09-07 — BOTH PARTS — and verified behaviourally.** It splits Client Meeting into two umbrellas, Old Meeting (`client_meeting_old`) and New Meeting (`client_meeting_new`); see the Meeting buckets section above for the rule. STEP 2 backfilled every pre-existing meeting as **Old** (the owner's ruling, chosen over per-row bucketing by each lead's stage TODAY — ~187 came from the legacy imports and their leads have moved on, so a per-row answer would read as real data while being fabricated); STEP 3 then **REMOVED the plain `client_meeting` value from the activities CHECK**, so no write path — app code, an import script, or a hand-typed INSERT — can create a meeting that neither umbrella counts. **It was deliberately run in two parts with the deploy in between: PART A (steps 1-2) → deploy → PART B (step 3), and that sequencing is the reusable lesson.** The breakage window is symmetric and easy to miss — migrate fully first and the still-live old build writes the now-outlawed `client_meeting`; deploy first and the new build writes a value the CHECK doesn't allow yet. PART A alone closes both, because it leaves all three values legal while the deploy rolls out. **Reach for this shape whenever a migration retires a value the running build still writes.** **Verified against the live database from an owner session, not by trusting the file**: an UPDATE of a real row back to `client_meeting` is refused with `23514` naming `activities_activity_type_check`, while both new values are accepted. `client_meeting_new` was checked deliberately with a reversible UPDATE-and-revert **because it has zero rows** — a typo dropping it from the CHECK would have let the ALTER succeed anyway and surfaced only later, in production, as New Meetings failing to save. Counts after: 0 unbucketed / 228 old / 0 new, unchanged by the probes. The end-to-end save was also exercised live from a real coordinator session between the two parts (a Client Meeting on a `calling` lead stored as `client_meeting_new`, success card reading "New Meeting", credited to the exec and logged-by the coordinator; test row deleted afterwards). Pre-migration that same save failed with the CHECK violation as an inline error with no partial write — which is what proved the app was genuinely sending the bucketed value. Safe to re-run (STEP 2 is a no-op now). Independent of every other migration — no policy, trigger or function touched, and new CHECK values inherit the table's existing grants and RLS. **It supersedes `migration_client_meeting_design_sheet.sql`'s section 1**, which now carries a reversion warning at its head: re-running that older file reverts the split AND would then reject every bucketed row already stored. `follow_ups.activity_type` is deliberately untouched (it still allows `client_meeting`; a reminder has no bucket yet). **Still open:** query 4d's stranded targets — any `targets` row still keyed on the retired combined `client_meeting` metric no longer computes an actual and needs re-entering against Old Meetings / New Meetings.
...
- **✅ `Schema/migration_needs_attention_hold_exclusion.sql` was RUN LIVE 2026-09-08 and verified via the RPC directly** (owner session, `supabase.rpc('leads_needing_attention', ...)`) — the returned rows now carry `last_stage_change_at`, and of 121 attention rows, **0** on-hold leads came back `is_stale: true`. Originally written from the owner's report that on-hold leads were showing up in the "stale" bucket with multi-month ages, which isn't fair since On Hold deliberately pauses a lead. Two fixes to `leads_needing_attention()`: (1) `is_stale` now excludes `current_stage = 'on_hold'` outright, however long the pause has run — no floored/reset age the way an imported lead's pre-history is clamped, it simply isn't judged while paused; (2) the "since touch" reference the stale gate tests is now `GREATEST(last activity, last stage change)`, not last activity alone — a stage change is as real a touch as a logged activity (LeadDetail's own health pill has always folded stage_history into its "last touched" calc this way), so the very stage_history row that takes a lead *off* hold resets this bucket's clock to the day of the resume rather than picking up wherever the pause left it. `src/lib/attention.js` ships the matching client-side fix in the same release (`buildLastStageChangeByLead`/`staleSinceTouch`/`staleDescription`, threaded into every `computeAttentionBuckets` call site — Dashboard's fallback, Home, OwnerToday, TeamTodayPanel, MyTeam, EmployeeProfile — via `fetchStageHistoryForFunnel`, already-fetched data reduced client-side, no new query), so the client-side fallback path and `LeadsListCard`'s "Nd silent"/EmployeeProfile's touch-color/LeadDetail's health pill (all three via `staleGateDays`'s new on-hold sentinel, see below) were correct immediately on release — Dashboard's fast RPC path needed this migration to match, and now does. This migration DROPs and recreates the function (adding `last_stage_change_at` to its return shape is a return-type change, which `CREATE OR REPLACE` can't do in place) — safe, since it's read-only with one caller. **Also fixed in the same release, no migration needed**: `staleGateDays` (`src/lib/attention.js`) now returns a "never stale" sentinel for any `current_stage = 'on_hold'` lead, and `LeadDetail.jsx`'s health pill and "Last touch" deal stat no longer show a staleness-colored day count for a **won, lost, or on-hold** lead at all (`showTouchHealth = isOpen && !isOnHold`) — previously both rendered regardless of stage, so opening a decided or paused deal still showed e.g. "Needs attention · 180d no touch". A real adjacent bug was fixed in the same block: a `lost` lead's status pill read "Open lead" (the `isAtRisk` branch excludes `won`/`lost` via `isOpen`, but nothing else in the ternary chain checked for `lost` specifically) — it now reads "Lost", styled like the other bad-tone pills. **Still worth a check as a real logged-in non-owner role** (the RPC fast path is what most sessions hit) if a resumed-from-hold lead's stale status is ever in doubt — the owner-session RPC probe above proves the shape and the on-hold exclusion, but not RLS-scoped behaviour, same "never trust the SQL Editor for that part" caveat the sibling RPC migration's VERIFY section spells out.
- **✅ `Schema/migration_stale_7day_tile.sql` was RUN LIVE 2026-09-08**, same day it was written — adds an `is_stale_7d` flag to `leads_needing_attention()`, gated on a new `p_stale_days` parameter (default 7), and widens the function's `WHERE` clause to `OR f.f_stale_7d` so a lead stale at 7-13 days (not otherwise matching any of the five existing buckets) is actually returned in the result set at all. Closes a real, reported display bug, not a hypothetical one: RightNowStrip's "Stale Leads" tile and `EmployeeProfile.jsx`'s "Stale leads" stat both used to read `leads_needing_attention()`'s existing `is_stale` (ATTENTION_DAYS/14) flag while labelled "7+ days" — see the Needs Attention entry above for the full writeup and the client-side fix (`computeStale7Bucket`/`computeStale7BucketFromRpc`, `src/lib/attention.js`), which shipped in the same release as this migration. **Verified via the migration's own verification query**, run by the owner in the SQL Editor: `is_stale_7d` is present in the function's output, and on a real row (lead #159) `is_stale`/`is_stale_7d` were both `true` together, confirming the superset relationship (nothing crossed the ATTENTION_DAYS/14 gate without also crossing the earlier STALE_DAYS/7 one). `EmployeeProfile.jsx` needed no migration either way (always client-side). Same DROP-and-recreate shape as `migration_needs_attention_hold_exclusion.sql` (adding a return column is a return-type change `CREATE OR REPLACE` can't do in place) — ran after that migration, reusing its exact on_hold-exclusion/`GREATEST(last activity, last stage change)` rule for the new flag rather than a second one. Safe to re-run. **Not yet verified from a real logged-in session** — the SQL Editor check above runs as `postgres` with BYPASSRLS, so it proves the function's shape and this one row's flags but nothing about RLS-scoped behaviour per role, same "never trust the SQL Editor for that part" caveat every sibling RPC migration's VERIFY section spells out — worth a look at the Dashboard "Right now" strip in a real session.
...
- **✅ `Schema/migration_rls_performance_leads_stage_history.sql` was RUN LIVE 2026-09-07, per the user — not yet independently re-verified in this session.** The user ran it and reported it done; the behavioral checks in the migration's own VERIFY section (same exec-session counts, coordinator/manager still can't see an outside lead, funnel avg-days no longer blank, `stage_history` request duration) have not been reported back — confirm those before trusting this as fully closed, per this file's own "verify against the live database, not against this file" rule. It was the fix for a defect found while fixing the 416 bug above: `stage_history` reads sit near the 8s statement timeout for a sales executive, and the sales funnel's "avg days in stage" column is blank because of it. `fetchStageHistoryForFunnel` measured **5.5s idle and timed out (57014) under a real Dashboard load** — for 88 rows. The funnel still renders its stage COUNTS (it seeds those from `breakdownLeads`), so the card looks populated; only the avg-days figures are missing, which is why this hid behind the bigger bug. **The diagnosis, and it is the interesting part: an exec's queries are slow in proportion to the rows they CANNOT see, not the rows they can.** `leads` SELECT is three permissive policies OR'd into effectively four boolean branches, and Postgres short-circuits an OR — so for a rep's own row the first test (`owner_employee_id = current_employee_id()`) matches and it is cheap, but for each of the ~1,100 rows belonging to someone else every branch is evaluated, including `is_my_team_member()` and `is_my_managed_member()`, two `SECURITY DEFINER` functions that each query `employees` **per row** (and can never be inlined away, since inlining a `SECURITY DEFINER` function would silently run it as the caller instead of the owner). That is why the OWNER (1,209 visible rows) is fast and an exec (86 visible rows) is slow, which looks backwards until you see it. `stage_history` pays it twice over — its own policy runs `EXISTS (SELECT 1 FROM leads ...)`, and the `leads(owner_employee_id)` embed runs it again. **The fix**, drafted after loading the `supabase-postgres-best-practices` skill per this bullet's own earlier instruction: hoist every bare `current_employee_id()`/`current_employee_role()` call (and, per that skill's own worked example, the argumented `is_my_team_member(x)`/`is_my_managed_member(x)` calls too) into `(select ...)` form, and guard each team predicate behind a cheap role test — e.g. `(select current_employee_role()) = 'sales_coordinator' AND (select is_my_team_member(owner_employee_id))` — so the whole branch folds to a constant `false` for an exec instead of calling into `employees` a thousand times; safe because both helpers already require the matching role internally, so `role = X AND helper(...)` is `A AND B` where `B` already implies `A` — a no-op on the truth table (proof spelled out per-policy in the migration file), pure win on execution. Scoped deliberately to just `leads`/`stage_history` (the two tables actually measured), not a schema-wide rewrite — every other table has the identical two defects and is flagged as a same-recipe follow-up inside the migration's own "DELIBERATELY NOT CHANGED" section, not fixed here, to keep this pass's reviewable surface small given the downside of a mistake is a data leak, not a blank card. **Must run LAST**, after `rls_policies.sql`, `migration_backlog_2026_08_10.sql`, `migration_owner_only_stage.sql`, `migration_scope_stage_history.sql`, `migration_sales_coordinator.sql`, `migration_lead_edit_rights.sql`, and `migration_sales_manager.sql` — it rewrites the final state those seven build up, so re-running any of them afterward silently reverts whichever policy they define back to the slower form (re-run this file again to restore it). The migration's own VERIFY section spells out why re-measuring must be done as a real logged-in sales executive (and separately as a coordinator/manager, checking a row known to be outside their team still returns zero), never from the SQL Editor — that runs as `postgres` with BYPASSRLS and no `auth.uid()`, so every policy here would evaluate false and look identical whether or not the fix worked. **Two lessons from the investigation, worth more than the fix itself.** (1) **The test suite was green throughout, because `fakeTable` in `fetchAllRows.test.js` modelled an out-of-range page as an empty array** — the fake server was wrong in exactly the way that mattered, so two tests literally named "is harmless when the table turns out to fit in one page" and "is harmless on an empty table" both passed while the real thing was catastrophic. The helper now returns a real 416; those two tests plus two new named regression cases all fail without the fix. **When a helper models a server, the model is load-bearing — check it against the real one's error behaviour, not just its happy path.** (2) **Row counts differ per role, so a perf change gated on row counts MUST be verified as the role with the FEWEST rows, not the most.** Verifying as the owner is verifying the one case that is guaranteed to work. This is the row-count sibling of the role × breakpoint rule at the top of this file, and note it also predates this fix: `LeadsListCard.jsx`'s own comment already recorded reproducing a live 416 from a stale page number, which is the same server behaviour that was then modelled away in the tests.
- **✅ `Schema/migration_rls_performance_parties_sites_activities.sql` was RUN LIVE 2026-09-07 and structurally verified** — the file's own VERIFY query #1 (all 20 policies across `parties`/`sites`/`activities`) returned `true`/`true` for every row. **Caught and fixed a real completeness gap on the first pass**: the initial draft missed `parties.coordinator_team_update`/`sites.coordinator_team_update`, defined in `Schema/migration_coordinator_entry.sql` (2026-08-11) — a file that wasn't part of the original grep sweep. The verify query's own broad policy-name list caught it (came back `false`/`false` for those two rows specifically), it was added as STEP 4 in the same file, and re-verified clean — worth remembering: **a "wrap every policy on table X" migration needs a grep across every file that has EVER touched table X's policies, not just the files already known to define its "main" policies** — a table's RLS can accumulate policies from migrations added for an unrelated feature months apart (here, a coordinator-entry-on-behalf-of-exec feature quietly added an UPDATE policy to two tables a performance pass had no reason to expect). **Not yet done**: the file's own VERIFY steps 2/3 — behavioral re-check as a real exec/coordinator/manager session (same row counts, still can't see an outside-team lead) and the Resource Timing re-measurement on `parties`/`sites` search. This was the sibling to the `leads`/`stage_history` fix above, prompted by a live report that party/site search-or-create (`PartySearchOrCreate`) and Log Activity's "Against which lead?" picker (`LeadSearchSelect`, which resolves against `parties`/`sites` before it ever touches `leads`) are slow for every non-owner role. Same `(select ...)` wrapping treatment, extended to `parties`/`sites`, **plus `activities`** — included not as a guess but because `parties.team_scoped_select` joins through `activities` in one of its own EXISTS branches, so `activities`' unwrapped, ungated `coordinator_team_*`/`manager_team_select` policies were being silently re-evaluated on every party search too. **Read the migration's own "WHAT THIS FILE CANNOT DO" section before expecting sibling-level gains**: unlike the team-only branches on `leads`, `parties`/`sites`' own-visibility checks ("is this contact linked to one of MY leads/activities/site-contacts") can't be skipped for an exec or manager — the honest answer is genuinely sometimes yes, sometimes no, so there's no role guard to hoist in front of it, only cheaper individual checks. Every EXISTS branch this touches is already backed by an existing index (`idx_leads_owner`/`idx_leads_party`/`idx_leads_referred_by`/`idx_leads_other_party`/`idx_leads_site`/`idx_activities_employee`/`idx_activities_party`/`idx_site_contacts_site`/`idx_site_contacts_party`/`idx_parties_created_by`/`idx_sites_discovered_by`, confirmed by reading `tostem_crm_schema.sql` directly) — so the ceiling here is real, cheap-but-nonzero per-row cost, not a missing index. Must run after `rls_policies.sql`/`migration_sales_coordinator.sql`/`migration_sales_manager.sql`, same reversion hazard as the sibling file. **If this doesn't fully resolve the reported slowness once run and re-measured, the next tier is a precomputed visibility table, not another round of wrapping** — see `PERFORMANCE.md`'s "next tier" table, added the same day.
...
- **Every Supabase request goes through `src/lib/supabaseFetch.js`** (wired in via `createClient`'s `global.fetch` in `supabaseClient.js`) — a retry + timeout wrapper. It exists because of a real reported bug (2026-09-02): on an **installed iPhone PWA**, saving a change to a lead sat for seconds and then failed with **"Load Failed"**, while pressing Save a second time saved instantly, every time; desktop was flawless on the same wifi. The cause was not the queries or the network's speed. iOS reaps idle HTTPS connections, and a request sent on a connection the OS has torn down rejects at the network layer — WebKit's wording for that is exactly `TypeError: Load failed`. **The reason only SAVES failed is a rule inside postgrest-js**: it retries idempotent requests (GET/HEAD/OPTIONS) 3× with 1s/2s/4s backoff and deliberately re-throws immediately for every other method, so the identical connection drop was merely *slow* on a page load — the other half of what was reported — and *fatal* on a Save. Desktop browsers retry a dead keep-alive connection internally; iOS surfaces it to the page. The rules, and don't loosen them casually: idempotent methods (GET/HEAD/OPTIONS/PUT/PATCH/DELETE) retry up to 3 attempts on either a network rejection or the 20s timeout; **POST retries once on a network rejection and NEVER on a timeout** — a rejection means the request was never delivered (which is why the rep's own second tap never produced a duplicate), while a timeout leaves genuinely open whether the server already ran it, and guessing wrong there writes a row nobody asked for. A caller's own abort (a superseded navigation) is never retried, and an HTTP 4xx/5xx is passed straight through — that's the server answering, not a dropped connection. Pinned by `supabaseFetch.test.js` (10 cases) and verified in a real browser: a genuine `supabase.from('leads').update()` was made to fail once with the exact iOS error and came back `error: null` after retrying, in 585ms. **Confirmed live on the reporting device 2026-09-02** — the owner re-tested on the installed iPhone PWA after this deployed and the first Save now lands on the first tap, so the dropped-connection diagnosis is observed, not just reasoned through. **`errorMessage.js` went with it** — a network failure that survives the retries now reads "Couldn't reach the server. Check your connection and try again." instead of surfacing WebKit's `TypeError: Load failed` verbatim to a sales exec. It deliberately does **not** claim "nothing was saved", since for an un-retried timed-out POST that isn't knowable. **The installed PWA only picks this up after a deploy plus a relaunch** — see the PWA installability section on the service worker's `skipWaiting()`.
