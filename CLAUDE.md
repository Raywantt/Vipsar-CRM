...
- **⚠️ `Schema/migration_client_meeting_buckets.sql` is OUTSTANDING (written 2026-09-07, not yet run against the live database)** — splits Client Meeting into the two umbrellas Old Meeting (`client_meeting_old`) and New Meeting (`client_meeting_new`), backfills every pre-existing meeting as **Old** (the owner's ruling, chosen over per-row bucketing by each lead's stage TODAY — ~187 of these came from the legacy imports and their leads have moved on, so a per-row answer would read as real data while being fabricated), and then **REMOVES the plain `client_meeting` value from the activities CHECK** so no write path — app code, an import script, or a hand-typed INSERT — can create a meeting that neither umbrella counts. See the Meeting buckets section above for the rule itself. **RUN IT IN TWO PARTS, WITH THE DEPLOY IN BETWEEN — PART A (steps 1-2) → deploy → PART B (step 3).** The breakage window is symmetric and easy to miss: migrate everything first and the still-live old build writes the now-outlawed `client_meeting`; deploy first and the new build writes a value the CHECK doesn't allow yet. PART A alone closes both, because it leaves all three values legal so either build can write while the deploy rolls out; PART B is unhurried and breaks nothing while it waits. **Until PART A runs, logging a Client Meeting FAILS** with `new row for relation "activities" violates check constraint "activities_activity_type_check"`, surfaced as a normal inline error with no partial write — **reproduced live 2026-09-07 from a real coordinator session**, which also confirmed the app is genuinely sending `client_meeting_new` rather than the retired value. Everything else already works pre-migration and was verified live: the Old/New rows on Dashboard's Activity card, the 8-column heatmap, and both metrics in Set-a-target (all reading 0 until the backfill runs, since existing rows still hold `client_meeting`). Safe to re-run (STEP 2 is a no-op afterwards). Independent of every other migration — no policy, trigger or function touched, and new CHECK values inherit the table's existing grants and RLS. It DOES supersede `migration_client_meeting_design_sheet.sql`'s section 1, which now carries a reversion warning at its head: re-running that older file reverts the split AND would then reject every bucketed row already stored. `follow_ups.activity_type` is deliberately untouched (it still allows `client_meeting`; a reminder has no bucket yet). **Confirm this has run, then verify by logging a real Client Meeting as a sales_executive** — the pre-migration failure above is the proof it hasn't.
