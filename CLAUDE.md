...
- **⚠️ `Schema/migration_targets_team_write.sql` is OUTSTANDING (written 2026-09-07, not yet run against the live database)** — adds `coordinator_team_insert`/`coordinator_team_update` and `manager_team_insert`/`manager_team_update` policies on `targets`, so a `sales_coordinator`/`sales_manager` can actually save a target for one of their own team's execs (see the Dashboard section's "Set a target" bullet). Closes a real gap, not a hypothetical one: `SetTargetForm.jsx`'s "+ Set a target" button has rendered for both roles since `seesOthersData` grew to include them during the Sales Manager build (2026-09-03) — `targets`' RLS never followed. Both roles already have a team-scoped SELECT policy on `targets` (`migration_sales_coordinator.sql`/`migration_sales_manager.sql`), so the heatmap already shows a team's existing targets correctly today; only INSERT/UPDATE were missing, so tapping "Set" currently fails with a 42501 RLS denial, surfaced inline like any other save error. Also rewrites the two existing `*_team_select` policies on `targets` into the `(select current_employee_role()) = '<role>' AND (select helper(...))` form the two 2026-09-07 performance migrations established, purely for consistency — `targets` is far too small a table for the per-row cost that form avoids to matter here. Safe to re-run (`DROP POLICY IF EXISTS` before every `CREATE POLICY`). Independent of both 2026-09-07 performance migrations (neither touches `targets`); must run after `migration_sales_coordinator.sql`/`migration_sales_manager.sql` (both already live), which it's guaranteed to since it only reuses their `is_my_team_member()`/`is_my_managed_member()` helpers rather than redefining them. **Confirm this has actually run, then verify as a real logged-in coordinator and a real manager — never from the SQL Editor** (see the migration's own VERIFY section for why that specifically would prove nothing) **— before relying on either role's "Set a target" button.**
...
- **⚠️ TWO migrations remain genuinely unconfirmed, and NEITHER can be checked from an owner session** (audited 2026-09-09): `migration_targets_team_write.sql` and `migration_manager_reassign_any_employee.sql`. Both are pure RLS policy changes, and PostgREST exposes no `pg_policies` — so the only proofs are behavioural, as the affected role. `targets_team_write` needs a real `sales_coordinator` or `sales_manager` to save a target for one of their own team's execs (an owner's write succeeds through the owner branch regardless, so testing as owner proves NOTHING); `manager_reassign_any_employee` needs a real `sales_manager` to reassign a lead to an exec **outside** their own team. Everything else in this list was probed directly and is live.
