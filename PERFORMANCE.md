...
| `parties`/`sites` search (search-or-create, Log Activity's lead picker) still feels slow after `migration_rls_performance_parties_sites_activities.sql`, most likely once party/site counts grow 10-20x from today's ~1,400 | **A precomputed visibility table** — e.g. `party_visible_to(party_id, employee_id)`, one row per (contact, employee) relationship that already exists, maintained by triggers on `leads`/`activities`/`site_contacts` the same way `lead_change_log` already maintains itself (see that migration for the pattern: a trigger, not app code, because there's no single write-path to hang a JS helper off). This is the scaled-down version of how large multi-tenant CRMs (Salesforce's sharing tables, etc.) actually do permissioned search — they precompute "who can see what" instead of recomputing it per query. `parties.team_scoped_select`'s 3-branch EXISTS/join collapses to one indexed lookup: `EXISTS (SELECT 1 FROM party_visible_to WHERE party_id = parties.id AND employee_id = current_employee_id())`. Real complexity trade-off (a new table, 3+ triggers, a one-time backfill), so don't build this ahead of the need — the RLS-wrapping fix already covers today's ~1,400-row reality. |
